...
class AddGuineeGamesMarketService {
  ...
  async syncMarkets() {
    console.log(`🚀 Fetching GuineeGames market groups and markets...`);

    const response = await fetchFromApi(this.apiUrl);
    if (!response) {
      console.warn(`⚠️ No data received from GuineeGames API.`);
      return;
    }
    ...
    console.log("✅ GuineeGames market groups & markets synced successfully!");
  }
...
export default new AddGuineeGamesMarketService();
