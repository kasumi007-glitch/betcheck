...
class FetchAkwaBetLeagueService {
    private readonly apiUrl =
        ...
        "https://sports-apipro.logiqsport.com/api/pregame/getPregameData?providerId=1&h24=false&lang=en&siteid=43";
    ...
    async syncLeagues() {
        ...
        const sport = response.Sports.find((s: any) => s.Name?.International === "Football");
        if (!sport) {
            console.warn(`⚠️ No Football data found in ${this.sourceName}.`);
            return;
        }

        if (!sport.Categories) {
            console.warn(`⚠️ No categories found for Football in ${this.sourceName}.`);
            return;
        }
