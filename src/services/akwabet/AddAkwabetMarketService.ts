...
class AddAkwabetMarketService {
  ...
  async syncMarkets() {
    console.log(`🚀 Fetching AkwaBet market groups and markets...`);

    const response = await fetchFromApi(this.apiUrl);
    if (!response) {
      console.warn(`⚠️ No data received from AkwaBet API.`);
      return;
    }
    ...
    console.log("✅ AkwaBet market groups & markets synced successfully!");
  }
...
export default new AddAkwabetMarketService();
