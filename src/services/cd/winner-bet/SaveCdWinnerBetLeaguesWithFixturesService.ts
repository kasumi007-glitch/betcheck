...
class SaveCdWinnerBetLeaguesWithFixturesService {
    private readonly leaguesApiUrl = "https://winner.bet/services/evapi/event/GetSportsTree?statusId=0&eventTypeId=0";
    private readonly fixturesApiUrl = "https://winner.bet/services/evapi/event/GetEvents?betTypeIds=-1&take=100&statusId=0&eventTypeId=0";

    async syncLeaguesAndFixtures() {
        console.log("🚀 Fetching CD Winner bet leagues and fixtures...");
        ...
        const filePath = `./files/cd/cd_winner_bet_leagues_fixtures_${dateStr}.json`;
    ...
    private async processLeague(categoryName: string, tournament: any, categoryId: string, jsonData: any) {
        ...
        const url = `${this.fixturesApiUrl}&leagueIds=${leagueId}`;
...
export default new SaveCdWinnerBetLeaguesWithFixturesService();