...
class AddParipesaMarketService {
  ...
  async syncMarkets() {
    console.log(`🚀 Fetching Paripesa market groups and markets...`);

    const response = await fetchFromApi(this.apiUrl);
    if (!response) {
      console.warn(`⚠️ No data received from Paripesa API.`);
      return;
    }
    ...
    console.log("✅ Paripesa market groups & markets synced successfully!");
  }
...
export default new AddParipesaMarketService();
