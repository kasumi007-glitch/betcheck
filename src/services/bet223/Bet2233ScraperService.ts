...
class Bet2233ScraperService {
  ...
  async scrape(): Promise<void> {
    ...
    for (const country of countryElements) {
      ...
      // Filter leagues based on active leagues in our DB for this country.
      const activeLeagues = [];
      for (const leagueName of leagues) {
        // Apply league name mapping
  ...
  private async processMatches(
    ...
    for (const matchHandle of matchHandles) {
      ...
      const eventDate = new Date(basicInfo.time);
      //   if (isNaN(eventDate.getTime())) {
      //     console.warn(
      //       `⚠️ Unable to parse event time for match: ${homeTeam} vs ${awayTeam}`
      //     );
...
export default new Bet2233ScraperService();
