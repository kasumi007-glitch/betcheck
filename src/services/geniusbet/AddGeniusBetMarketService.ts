...
class AddGeniusBetMarketService {
  ...
  async syncMarkets() {
    console.log(`🚀 Fetching GeniusBet market groups and markets...`);

    const response = await fetchFromApi(this.apiUrl);
    if (!response) {
      console.warn(`⚠️ No data received from GeniusBet API.`);
      return;
    }
    ...
    console.log("✅ GeniusBet market groups & markets synced successfully!");
  }
...
export default new AddGeniusBetMarketService();
