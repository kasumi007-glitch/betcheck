...
class AddBetPawaMarketService {
  ...
  async syncMarkets() {
    console.log(`🚀 Fetching BetPawa market groups and markets...`);

    const response = await fetchFromApi(this.apiUrl);
    if (!response) {
      console.warn(`⚠️ No data received from BetPawa API.`);
      return;
    }
    ...
    console.log("✅ BetPawa market groups & markets synced successfully!");
  }
...
export default new AddBetPawaMarketService();
