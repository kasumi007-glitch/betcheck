...
class AddMelBetMarketService {
  ...
  async syncMarkets() {
    console.log(`🚀 Fetching MelBet market groups and markets...`);

    const response = await fetchFromApi(this.apiUrl);
    if (!response) {
      console.warn(`⚠️ No data received from MelBet API.`);
      return;
    }
    ...
    console.log("✅ MelBet market groups & markets synced successfully!");
  }
...
export default new AddMelBetMarketService();
