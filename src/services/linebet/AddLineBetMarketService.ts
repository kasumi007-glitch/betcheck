...
class AddLineBetMarketService {
  ...
  async syncMarkets() {
    console.log(`🚀 Fetching LineBet market groups and markets...`);

    const response = await fetchFromApi(this.apiUrl);
    if (!response) {
      console.warn(`⚠️ No data received from LineBet API.`);
      return;
    }
    ...
    console.log("✅ LineBet market groups & markets synced successfully!");
  }
...
export default new AddLineBetMarketService();
